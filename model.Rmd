...
#lattice::dotplot(ranef(M5,which="cognate",condVar=TRUE))
```
...
```{r}
c1 <- c("eng","spa")
coef<- c("0.70146","1.16337")
se <- c("0.17812","0.17547")
plot <- data.frame(c1,coef,se)

ggplot(data=plot,aes(x=coef,y=se))
```
...
```
```{r}
M7 <- brm(accuracy~cognate+(1|subject)+diff,data=spa2,family=bernoulli,prior = c(set_prior("normal(0,8)")))
print(summary(M7))
```

```{r}
M8 <- brm(accuracy~cognate+(1|subject)+diff,data=eng2,family=bernoulli,prior = c(set_prior("normal(0,8)")))
print(summary(M8))
```
